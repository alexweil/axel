...
4. **RECAP** (estructura y aviso: skill `recap` — llegás por trabajo autónomo, va push) y esperá el OK humano. Con el OK: registralo en STATUS y commiteá; el siguiente paso es `/plan` en sesión limpia — si la sesión tiene herramienta de spawn de sesión (hoy: el chip del desktop), creá el chip con prompt **únicamente `/plan`**; si no está o falla, instrucción única: sesión nueva + `/plan` (desde terminal: `claude "/plan"`).
