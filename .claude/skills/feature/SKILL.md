...
3. **Loop del padre**, por cada feature autorizado en orden:
   ...
   - **Lanzá el hijo**: subagente fresco en background **con modelo `opus`** — el modelo de los hijos lo fija la maquinaria por tipo de unidad, no la sesión, y los que corren `feature` van con `opus`. La tabla canónica, la naturaleza de los alias, la precedencia y el camino ante un rechazo del harness viven en la skill `build`, §«Modelos por unidad»; en un lote cambian solo dos cosas: el registro va al **ledger del lote** y el override del humano entra como **corrección del gate de lote** (paso 2), en la línea «Exclusiones/correcciones» del ledger — desde donde gana sobre `opus` también al relanzar por reentrada. Prompt mínimo sin contexto del chat: «Modo hijo del lote: feature NN, token \<nonce\>. Ledger: docs/implementation/batch-<fecha>.md. Leé docs/STATUS.md, AGENTS.md, docs/IMPLEMENTATION.md y el ledger, y seguí la sección "Modo hijo" de la skill feature.»
