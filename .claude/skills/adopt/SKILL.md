...
5. **Re-verificar lo mecánico** antes de cerrar: `CLAUDE.md` es symlink a `AGENTS.md`; `.claude/settings.json` tiene los permisos del loop y `defaultMode: acceptEdits`; los cuatro docs canónicos existen; `.gitignore` cubre `.claude/state/`. Cualquier faltante se resuelve ahora.
