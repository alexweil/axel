...
- **Ronda de review**: 6 completada (`CHANGES_REQUESTED`, 2 puntos de métricas — frontera de ciclo con retry de `new`, y `log_event` best-effort que jamás altera veredicto/RC — ambos corregidos con regresión); paso C commiteado (`tests/lint.sh` fail-closed + shellcheck 0.11.0, 4 warnings corregidos sin `disable`, triage de 90 info en el doc del feature); lanzando ronda 7 contra los criterios de cierre
