...
3. **Cierre**: con el APPROVED de cierre deja «APPROVED — pendiente OK de lote» en IMPLEMENTATION + doc del feature + STATUS, commitea, y termina con última línea `FEATURE <NN> APROBADO`. Sin RECAP individual, sin push, sin chip. Ante condición de corte propia (r6): registra el detalle en el doc del feature, **commitea y deja el árbol limpio** — el handshake exige frontera limpia: nada del hijo sin commitear — y termina con última línea `CORTE: <motivo breve>`.
...
- **Cortes** (r5/r6): ante `CORTE:` del hijo o corte propio, el padre **verifica el árbol antes de registrar**: si el hijo dejó cambios sin commitear (staged o unstaged — violación del handshake de frontera limpia), no los absorbe — registra el corte en el ledger y commitea **restringido por pathspec** (`git commit --only -- <ledger>`), que preserva intacta la suciedad, y la reporta en el RECAP del corte.
...
**Reentrada** (padre muerto, sesión nueva): STATUS apunta al ledger ⇒ la sesión lee STATUS + ledger + git y aplica el protocolo fail-closed del diseño. **Primero el ledger** (r6): un corte registrado sin resolución humana posterior es **absorbente** — precede a la máquina de estados del ancla: no se relanza nada aunque ancla y terminal matcheen (coherente con la conservación del ancla como evidencia ante corte); se re-presenta el RECAP del corte y se espera al humano. Sin corte pendiente, y a diferencia del watcher vivo — que ignora no-matches y espera — la reentrada **valida y decide** (r1), con `.claude/state/batch-expected` como ancla, cubriendo los tres estados del ciclo de vida (r2):
...
- **Paso B — skills + reglas** (2026-07-28): skill `feature` reescrita con el modo lote — dispatch por argumento, camino de re-presentación «esperando autorización de lote» (análogo al gate individual: STATUS con frase literal + commit antes de presentar), validación fail-closed, gate de lote con `gate_base` anotado al autorizarse (previo al commit del ledger), loop del padre (pre-arranque con chequeo de divergencia, lanzamiento del hijo con prompt mínimo, watcher con ejemplo ejecutable, empujón sin contenido, verificación entre features, cortes, mensajes humanos con bajada inmediata), modo hijo (los tres deltas + ciclo de vida de `batch-expected`), reentrada con los cuatro estados del ancla, y esqueleto del ledger inline; el flujo individual quedó **textualmente intacto** (pasos 1–8 y reglas del loop, con las dos adiciones mínimas: cierre consolidado en el camino «esperando OK» y renovación de awake por feature del lote). Skill `recap` con la sección «RECAP consolidado» (base `gate_base`, estructura por feature, no-revisados como siempre, excepción del commit de registro) y el lote sumado a los ejemplos de espera autónoma. `AGENTS.md` + `templates/AGENTS.md` sincronizados (regla dura redefinida + modo lote en la línea de `/feature`; el template referencia la skill y el contrato instalados, no docs de axel). Línea «Cómo se trabaja un feature» en `IMPLEMENTATION.md` + `templates/IMPLEMENTATION.md`. DESIGN.md sin cambios: ya reflejaba el modo lote desde el ciclo de diseño y la bajada no lo desvió.
...
- **r3** (2026-07-28, **`APPROVED`**, rango `0c2ee58..7de9cd9`): bajada fina aprobada — el ciclo `launched → consumed → borrado` cubre los puntos de caída, la ausencia del ancla ya no se interpreta aislada, las correcciones anteriores permanecen cerradas. Base a `7de9cd9`.
- **r4** (2026-07-28, **`APPROVED`**, rango `7de9cd9..2dbbdfc`): paso A aprobado — «la señal terminal cumple la bajada: clasificación completa, identidad, publicación atómica y preservación del RC/estado»; Codex indujo un fallo real de publicación (RC preservado, sin temporales) y corrió las suites por su cuenta (loop 287 ok en su sandbox — 6 asserts no contractuales omitidos —, install 459 ok, lint limpio). Observación no bloqueante: L10 son 9 casos nombrados (dos coberturas comparten caso), cobertura completa presente. Base a `2dbbdfc`.
- **r5** (2026-07-28, `CHANGES_REQUESTED`, rango `2dbbdfc..3b613e7`): paso B con cuatro puntos, los cuatro aceptados — (1) el payload instalado referenciaba `batch-features.md`, que no viaja (Codex lo confirmó con una instalación descartable) ⇒ skill `feature` y `review-contract.md` autocontenidos, con la referencia al diseño calificada como doc del repo axel fuera del payload; (2) la re-presentación del gate de lote volvía ambiguamente al «paso 2» (habría re-pedido autorización) ⇒ salta explícitamente al bloque «Con la autorización»; (3) el corte no cerraba la frontera padre↔hijo ⇒ el hijo commitea y deja el árbol limpio **antes** del handshake `CORTE:`, el padre verifica el árbol y ante suciedad commitea solo el ledger registrándola en el RECAP, y el corte registrado en el ledger es **absorbente** en la reentrada — precede a la máquina de estados del ancla; (4) docs no al día para un pedido de cierre (STATUS hablaba de r4, Review log cortado en r2) ⇒ este registro y STATUS al estado real.
- **r6** (2026-07-28, `CHANGES_REQUESTED`, rango `2dbbdfc..bf59b35`): validado el grueso de r5 (salto exacto, corte absorbente en la skill, docs actuales, payload sin links rotos — instalación descartable repetida); dos puntos, ambos aceptados — (1) «add explícito + commit» no garantizaba «solo el ledger»: con cambios *staged* del hijo, el commit normal los arrastra (Codex lo reprodujo con `--dry-run` y validó el aislamiento de `git commit --only -- <ledger>`) ⇒ commit restringido por pathspec en skill y bajada; (2) las correcciones de r5 quedaron solo en la skill: la bajada (secciones normativas del hijo y la reentrada) y la fila 07 de IMPLEMENTATION seguían desalineadas ⇒ delta 3 del hijo con frontera limpia, bullet de cortes del padre, precedencia absorbente en la reentrada de la bajada, y fila 07 al estado real («implementación en revisión de cierre»).
- **r7** (2026-07-28, `CHANGES_REQUESTED`, rango `2dbbdfc..21b9e66`): todo lo demás cerrado («bajada y skill alineadas en frontera limpia, corte absorbente y commit restringido; fila 07, STATUS y Review log actualizados»); un bloqueante trivial, aceptado — el comando literal de la skill tenía `-m` después del `--` y git lo interpreta como pathspec (repro del reviewer con `--dry-run`) ⇒ `git commit --only -m "…" -- <ledger>`, con la advertencia del orden anotada en la skill.
- **r8** (2026-07-28, **`APPROVED` de cierre**, rango `2dbbdfc..2d5d30d`): el comando corregido devuelve RC 0 y el `--dry-run` de Codex incluye únicamente el ledger preservando el cambio ajeno staged; «skill, bajada, IMPLEMENTATION, STATUS y Review log están alineados. Los seis criterios de cierre quedan satisfechos». Base a `2d5d30d`. Ciclo del feature: 8 rondas (r3 APPROVED bajada · r4 APPROVED paso A · r8 APPROVED cierre).
