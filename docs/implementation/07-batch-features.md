...
2. **Reviews**: genera el id único (§1), escribe `.claude/state/batch-expected` (id + head + feature — el ancla de la reentrada, abajo), invoca `review.sh` en background con `AXEL_REVIEW_ID=<id>` y **termina el turno** con última línea exacta: `REVIEW LANZADA id=<id> head=<sha del HEAD commiteado>`. Lo despierta el empujón del padre; el desenlace autoritativo es el **terminal** (r1): lee `result`/`rc` de `.claude/state/review-terminal` verificando su identidad, y solo ante `APPROVED` o `CHANGES_REQUESTED` consume `last-verdict` y `last-review.md` como vigentes — ante `PROC_FAIL`, `NO_VERDICT`, `DEADLOCK`, `INPUT_ERROR` o `ABORTED` esos archivos quedaron deliberadamente viejos, y el camino es el de fallas del loop vigente (diagnóstico, relanzamiento único si es claramente transitorio, o condición de corte).
...
**Reentrada** (padre muerto, sesión nueva): STATUS apunta al ledger ⇒ la sesión lee STATUS + ledger + git y aplica el protocolo fail-closed del diseño. A diferencia del watcher vivo — que ignora no-matches y espera — la reentrada **valida y decide** (r1), con `.claude/state/batch-expected` como ancla de la última review lanzada: sin `batch-expected` ⇒ no había review en vuelo — consistencia por ledger + git como en el diseño; con `batch-expected` y un terminal cuya identidad completa lo matchea ⇒ la review terminó sin empujón entregado — consistente: relanza un hijo fresco que retoma leyendo el terminal; con `batch-expected` y un terminal ausente o de otra identidad ⇒ review posiblemente en vuelo o estado desconocido ⇒ **inconsistencia**: RECAP con lo encontrado, sin relanzar. `batch-expected` vive en `.claude/state/` (estado local no versionado, como el resto del estado del loop): la reentrada remota sin ese estado cae al camino conservador. Nunca se re-pide el gate autorizado ni se re-cierra lo aprobado.
...
| Empujón / línea de estado | `REVIEW LANZADA id=<NN>:r<M>:<nonce> head=<sha>` / empujón sin contenido con el id; `FEATURE <NN> APROBADO`; `CORTE: <motivo>`; ancla de reentrada en `.claude/state/batch-expected` — §3 |
