...
- **r5** (2026-07-28, `CHANGES_REQUESTED`, rango `2dbbdfc..3b613e7`): paso B con cuatro puntos, los cuatro aceptados — (1) el payload instalado referenciaba `batch-features.md`, que no viaja (Codex lo confirmó con una instalación descartable) ⇒ skill `feature` y `review-contract.md` autocontenidos, con la referencia al diseño calificada como doc del repo axel fuera del payload; (2) la re-presentación del gate de lote volvía ambiguamente al «paso 2» (habría re-pedido autorización) ⇒ salta explícitamente al bloque «Con la autorización»; (3) el corte no cerraba la frontera padre↔hijo ⇒ el hijo commitea y deja el árbol limpio **antes** del handshake `CORTE:`, el padre verifica el árbol y ante suciedad commitea solo el ledger (add explícito, jamás `-A`) registrándola en el RECAP, y el corte registrado en el ledger es **absorbente** en la reentrada — precede a la máquina de estados del ancla; (4) docs no al día para un pedido de cierre (STATUS hablaba de r4, Review log cortado en r2) ⇒ este registro y STATUS al estado real.
