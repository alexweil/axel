...
- **El éxito se arma, no se deduce del RC** (punto 1 de la r1). Una descarga **parcial** es peor que una vacía: `curl` puede entregar un prefijo sintácticamente válido que ya instaló el trap y termina en EOF con estado 0 — un trap ingenuo firmaría `rc=0` sin que el instalador haya existido. Por eso una variable de completitud se arma **únicamente en los puntos terminales reales** (los `exit 0`/`exit 1` finales del instalador, la propagación del RC del delegado en el wrapper, y los rechazos explícitos de `die`/uso), y el trap `EXIT` decide con ella:
  - armada ⇒ `── axel · fin: rc=<N> · <motivo> ──` por stdout;
  - **no** armada ⇒ línea de incompletitud por stderr (`sin finalización confirmada — ¿script truncado o descarga parcial?`) y **RC forzado a 2** si el observado era 0 (si ya era ≠0 se conserva: no se inventa un error distinto del real).
...
- **Alcance honesto del contrato** (punto 2 de la r1): la línea cubre toda **salida controlada** del proceso, no "todos los caminos". Un `trap EXIT` no corre ante `TERM`/`INT` (el reviewer lo verificó: 143/130 sin salida), y componer manejadores de señal desde el arranque **se descarta a propósito**: haría que una muerte por señal del delegado terminara en un RC 2 contractual, que el wrapper propagaría *sin* su aviso de interrupción — perdiendo justo la información de que puede haber diff parcial — y tocaría el protocolo de lock del 02 (reenvío de `TERM` + `wait`) para un caso que el one-liner no necesita. Se acota la afirmación en su lugar: **ausencia de la línea = no hay finalización confirmada** (descarga fallida o parcial, o corrida interrumpida), nunca "terminó bien". La interrupción sigue cubierta como hasta hoy por el aviso de RC anómalo del wrapper.
...
- **T16j línea final**: rc 0 (T16a), rc 1 (adopción vía one-liner corto, con handoff), rc 2 (rechazo del bootstrap y rechazo del delegado por árbol sucio) — prefijo presente, RC correcto y **una sola** ocurrencia, también en modo local. **Skew de versiones**: delegado viejo (stub sin línea final) bajo wrapper nuevo, y delegado nuevo bajo un wrapper viejo simulado (que no exporta el marcador) → exactamente una línea en ambos. **Señal**: corrida muerta por `TERM` → **sin** línea final (el contrato acotado se cumple: no hay finalización confirmada).
- **T16k transporte**: descarga fallida entera (`sh -c 'exit 22' | bash` y `printf '' | bash`) → RC 0 y salida **sin** el prefijo final (el chequeo documentado lo detecta); **descarga parcial** — el prefijo más largo del propio `install.sh` que corta justo después de instalar el trap, verificado con `bash -n` y ejecutado por stdin — → línea de incompletitud y **RC 2** (no un falso `rc=0`), sin tocar el destino; la variante `bash -o pipefail -c '… | bash'` sobre el fallo de descarga → RC 22 (la remediación documentada funciona).
...
6. **Fallo del transporte resuelto**: el comando exacto se conserva; toda **salida controlada** termina con `── axel · fin: rc=N · …`, **una sola vez** (skew de versiones incluido), y el éxito solo se firma desde un punto terminal real: una descarga parcial que ejecute un prefijo válido produce la línea de incompletitud y **RC 2**, no un falso 0. La guía publica el chequeo —ausencia de línea o línea de incompletitud ⇒ no completó, nunca "terminó bien"— y la variante `pipefail`; los tests cubren descarga vacía, descarga parcial, muerte por señal y la variante que sí propaga el fallo.
