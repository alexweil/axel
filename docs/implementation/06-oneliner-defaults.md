...
- **Las cinco fronteras de la detección** (r3), que son las que criterios, riesgos y matriz deben decir sin exagerar:
  ...
  | corte **después** del trap y antes de un `finish` | 0 ⇒ **forzado a 2**; un RC no-cero real se **conserva** (p. ej. 1 por `set -e`) | incompletitud (stderr) |
  ...
  | señal **no capturada** (modo local, o wrapper antes del lock) | 128+n | ninguna — se detecta por ausencia |

  El caso pre-trap es irreducible: hasta que el prefijo no instala el trap no hay código nuestro corriendo —el RC lo decide bash y no significa nada sobre la instalación—, y por eso el chequeo del caller —ausencia de línea— es la capa que lo cubre, la misma que cubre la descarga vacía. En la región post-trap el trap **no inventa** un error: solo convierte el 0 engañoso en 2 y deja pasar cualquier RC no-cero real, que ya es una señal honesta de falla.
