...
- **Las cinco fronteras de la detección** (r3), que son las que criterios, riesgos y matriz deben decir sin exagerar:
  ...
  | situación | RC | línea |
  |---|---|---|
  | corte **antes** de que el prefijo instale el trap (incluida la descarga vacía) | 0 | ninguna — se detecta por **ausencia** |
  | corte **después** del trap y antes de un `finish` | 0 ⇒ **forzado a 2** | incompletitud (stderr) |
  ...
  El caso pre-trap es irreducible: hasta que el prefijo no instala el trap no hay código nuestro corriendo, y por eso el chequeo del caller —ausencia de línea— es la capa que lo cubre, la misma que cubre la descarga vacía.
...
- **T16k transporte y completitud**: descarga fallida entera (`sh -c 'exit 22' | bash` y `printf '' | bash`) → RC 0 y salida **sin** el prefijo final (el chequeo documentado lo detecta); **descarga parcial** en las dos regiones, con los prefijos verificados por `bash -n` y ejecutados por stdin: cortada **justo antes de una llamada terminal a `finish`** → línea de incompletitud y **RC 2**, sin tocar el destino; y cortada **antes de que el prefijo instale el trap** (prefijo no vacío, p. ej. solo el encabezado y las primeras funciones) → RC 0 **sin** línea, que es la frontera declarada y la cubre el mismo chequeo de ausencia que la descarga vacía; **invariante por inspección**: los únicos `exit` del script viven en `finish` y en el trap, y la única asignación de completitud vive en `finish` (si alguien agrega un terminal suelto, el test falla antes de que el contrato se rompa en producción); la variante `bash -o pipefail -c '… | bash'` sobre el fallo de descarga → RC 22.
...
6. **Fallo del transporte resuelto**: el comando exacto se conserva; toda **salida controlada** termina con `── axel · fin: rc=N · …`, **una sola vez** (skew de versiones incluido), y el éxito solo se firma desde el primitivo terminal `finish`, que arma y sale indivisiblemente: una descarga parcial cortada **después** de que el prefijo instale el trap —incluido justo antes de una llamada terminal— produce la línea de incompletitud y **RC 2**, nunca un falso `rc=0`; cortada **antes** del trap sale 0 sin línea y la cubre el chequeo de ausencia del caller, igual que la descarga vacía (las cinco fronteras están tabuladas en el enfoque técnico y la matriz cubre las dos regiones). Todo terminal intencional del script pasa por el primitivo, con la invariante verificada por inspección además de por comportamiento. La guía publica el chequeo —ausencia de línea o línea de incompletitud ⇒ no completó, nunca "terminó bien"— y la variante `pipefail`; los tests cubren descarga vacía, descarga parcial, muerte por señal y la variante que sí propaga el fallo.
