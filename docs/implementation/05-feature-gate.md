...
- **Paso único de implementación** (2026-07-28): delta solo-prosa según el mapa de "Docs afectados". Skill `feature`: sección nueva «Si STATUS dice "esperando confirmación de arranque"» (re-presentación: re-derivar de las mismas fuentes, procedencia respuesta directa con remisión a `recap`, continuar en el paso 3 con la confirmación) y el camino "Feature nuevo" renumerado 1–8 — el gate como paso 2 (derivación con fallback honesto, STATUS con la frase literal + commit **antes** de presentar, fin de turno) y el registro de la confirmación como paso 3 (fecha + literal breve + correcciones, viaja en el commit de la bajada; no-confirmación → prioridad absoluta del humano, `/plan` si invalida el feature); los pasos 4–8 idénticos a los 2–6 previos (flujo post-confirmación sin cambios, OK final intacto). Skill `recap`: la lista interactiva del protocolo gana «la presentación o re-presentación del gate de arranque de `/feature`» — único cambio en esa skill, sin tocar el criterio ni el formato. `AGENTS.md` + `templates/AGENTS.md`: línea de `/feature` con el gate, idéntica en ambos tras el número de ítem (verificado por diff). `IMPLEMENTATION.md` + `templates/IMPLEMENTATION.md`: línea "Cómo se trabaja un feature" con el gate, misma formulación en ambos (verificado por diff). `DESIGN.md`: gate en el diagrama del flujo + fila de decisión 2026-07-28 con remisión a este doc. `scripts/`, `tests/`, `review-contract.md` y skills `design`/`plan`/`adopt`/`status`: intactos.
- **Tests** (2026-07-28, HEAD del paso): `tests/lint.sh` limpio (shellcheck 0.11.0, severidad warning); `tests/install.sh` 321 ok · 0 fail (las skills modificadas son payload del instalador — el re-run las propaga); `tests/loop.sh` 244 ok · 0 fail; `git diff --check` limpio.

## Review log

- **Ronda 4: APPROVED (cierre)** — el punto de r3 cerrado (fila 05 de IMPLEMENTATION coincidente con STATUS y el Review log); **los cinco criterios de cierre satisfechos** a juicio del reviewer, que verificó además que el commit incremental solo toca los tres docs declarados, las superficies implementadas idénticas, y `git diff --check` + lint limpios. APPROVED de cierre del feature 05. Base: `c695431`. Convergencia del ciclo: bajada 2→0 (r1–r2), implementación 1→0 (r3–r4).
- **Ronda 3 (implementación): CHANGES_REQUESTED** (1 punto, aceptado; el reviewer dio por satisfechos los demás criterios — gate completo, pasos posteriores preservados, protocolo sin restatement, duplicaciones sincronizadas, DESIGN y bootstrap correctos — y verificó por su cuenta `git diff --check` + lint limpios, `tests/install.sh` 321/0 y el fuera-de-alcance intacto). Resolución:
  1. La fila 05 de IMPLEMENTATION.md quedó desactualizada ("bajada fina en review" pese al APPROVED de r2 con la implementación ya en review), bloqueando el criterio 5 → corregido: fila con bajada APPROVED (r2) e implementación en review — la memoria oficial refleja el estado actual.
- **Ronda 2: APPROVED (bajada)** — los 2 puntos de r1 cerrados a juicio del reviewer (bootstrap registrado fielmente sin literalidad retroactiva; criterio 2 acotado a las skills de fase); STATUS y Review log al día; verificación independiente: `git diff --check` y `tests/lint.sh` limpios, commit incremental solo sobre los dos docs declarados. Base movida a `3306704`. Habilita la implementación; no cierra el feature.
