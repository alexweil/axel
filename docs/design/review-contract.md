...
- `id` es la **identidad de invocación de una corrida orquestada** — el modo lote de `/feature` y el pipeline de `/build` (protocolos operativos: skills `feature` y `build`; los diseños de fondo viven en el repo axel como `design/batch-features.md` y `design/implicit-entry.md`, fuera del payload instalado): el invocador la pasa por env con unicidad real por invocación, `<unidad>:r<M>:<nonce>`, donde la unidad es el número del feature (`NN`) en un lote, y `NN`, `design` o `plan` según el tipo de unidad en un pipeline. `review.sh` **publica el valor sin interpretarlo**: el formato es contrato entre invocador y consumidor, no lógica del script. Fuera de una corrida orquestada no se setea (`id=-`) y nada del flujo cambia; ese terminal es el que consume la **reentrada individual** de cualquier fase, con la identidad que define §Reentrada.
...
La línea **«Esperando»** usa vocabulario fijo: «esperando OK humano», «esperando confirmación de arranque», «esperando autorización de lote», «esperando confirmación de plan», «esperando autorización de pipeline» (las cinco esperas **humanas**: las únicas que disparan re-presentación y aviso), «esperando el desenlace de la review (ronda N)», o «nada del humano — \<trabajo en curso\>». La cuarta la fija el ruteo implícito —`/plan` despachado por contexto no tiene gate propio— y es la única que **lleva contenido en la línea**: la interpretación del pedido, que no es derivable de los docs porque su fuente era el chat. La quinta tampoco es derivable —la ruta de un pipeline sale del pedido del humano—, pero no cabe en una línea: se persiste en el **ledger de la corrida**, creado *antes* de presentar el gate, y STATUS solo lo apunta. Las otras tres re-derivan su presentación de los docs.
